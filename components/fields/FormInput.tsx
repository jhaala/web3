...
const FormInput: React.FC<FormInputType> = ({ name, value, placeholder, type, error, className = '', onChange, ...rest }) => {
  ...
  return (
    <div className='my-3'>
      <div
        className={classNames('gap-4 px-4 text-gray-900 rounded-2xl flex-center bg-slate-100', {
      ...
      >
        <input
          name={name}
          type={type}
          className={`relative focus:outline-none ring-0 block py-4 w-full bg-slate-100 placeholder-gray-700 rounded-2xl appearance-none sm:text-sm ${className}`}
